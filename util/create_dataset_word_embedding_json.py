...
if __name__ == '__main__':
    ...
    for line in tqdm(cur_latest_json_lines):
        bug = ujson.loads(line) #current bug
        ...
        if int(bug['bug_id']) > int(training.bugIds[-1]): #print if current bugs bugid is greater than the last bug in training, but this doesnt effect overall word embedding
